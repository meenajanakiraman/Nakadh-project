...
@Component({
    ...
    <div class="card">
      <div class="flex justify-between items-center mb-4 gap-2 flex-wrap">
        <h2 class="text-pink-600 text-2xl font-bold">new Members</h2>
      ...
      <p-table
        ...
        <ng-template pTemplate="body" let-member let-i="rowIndex">
          <tr>
            ...
            <td><button pButton type="button" (click)="modifyUserStatus('Inactive', member.email)" label="Deactivate" class="p-button-secondary p-button-sm"></button></td>
...
export class MemNewmembersComponent implements OnInit {
    newMembers: any[] = [];
    totalRecords = 0;
    page = 1;
    limit = 10;
    ...
    loadUsers(page: number, limit: number, globalFilter: string = '') {
        ...
        this.loading = true;
        this.allUserDetailsService.getallNewUsers(page, limit, globalFilter).subscribe({
            next: (data: any) => {
                this.newMembers = data.data
    ...
    exportToExcel() {
        ...
        this.saveAsFile(excelBuffer, 'newMembers', 'xlsx');
    }
    ...
    exportToCSV() {
        ...
        FileSaver.saveAs(blob, `newMembers_${new Date().getTime()}.csv`);
    }

    exportToPDF() {
        const doc = new jsPDF();
        const headers = [['Reg No', 'Name', 'Email', 'Mobile']];
        const rows = this.newMembers.map(m => [m.registernumber, m.name, m.email, m.mobilenumber]);
        autoTable(doc, { head: headers, body: rows });
        doc.save(`newMembers_${new Date().getTime()}.pdf`);
    }
