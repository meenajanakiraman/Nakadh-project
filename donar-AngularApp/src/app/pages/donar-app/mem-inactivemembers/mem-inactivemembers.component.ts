...
@Component({
    selector: 'app-mem-inactiveMembers',
     standalone: true,
    ...
    <div class="card">
      <div class="flex justify-between items-center mb-4 gap-2 flex-wrap">
        <h2 class="text-pink-600 text-2xl font-bold">Inactive Members</h2>
...
export class MemInactivemembersComponent implements OnInit {
    inactiveMembers: any[] = [];
    totalRecords = 0;
    page = 1;
    limit = 10;
    ...
    loadUsers(page: number, limit: number, globalFilter: string = '') {
        ...
        this.loading = true;
        this.allUserDetailsService.getallUsers(page, limit, globalFilter,'Inactive').subscribe({
            next: (data: any) => {
                this.inactiveMembers = data.data
    ...
    exportToExcel() {
        ...
        this.saveAsFile(excelBuffer, 'inactiveMembers', 'xlsx');
    }
    ...
    exportToCSV() {
        ...
        FileSaver.saveAs(blob, `inactiveMembers_${new Date().getTime()}.csv`);
    }

    exportToPDF() {
        const doc = new jsPDF();
        const headers = [['Reg No', 'Name', 'Email', 'Mobile']];
        const rows = this.inactiveMembers.map(m => [m.registernumber, m.name, m.email, m.mobilenumber]);
        autoTable(doc, { head: headers, body: rows });
        doc.save(`inactiveMembers_${new Date().getTime()}.pdf`);
    }
    ...
    private formatExportData() {
        return this.inactiveMembers.map(m => ({
