...
@Component({
    selector: 'app-mem-activemembers',
    standalone: true,
    providers: [MessageService],
    imports: [Toast, TableModule, ButtonModule, TagModule, InputTextModule, RouterModule, FormsModule],
    template: `
    <div class="card">
      <div class="flex justify-between items-center mb-4 gap-2 flex-wrap">
        <h2 class="text-pink-600 text-2xl font-bold">Active Members</h2>
      ...
      <p-table
        ...
        <ng-template pTemplate="body" let-member let-i="rowIndex">
          <tr>
            ...
            <td><button pButton type="button" (click)="modifyUserStatus('Inactive', member.email)" label="De-Activate" class="p-button-secondary p-button-sm"></button></td>
            <td><button pButton type="button" (click)="modifyUserStatus('Blocked', member.email)" label label="Block" class="p-button-danger p-button-sm"></button></td>
          </tr>
        </ng-template>
      </p-table>
...
export class MemActivemembersComponent implements OnInit {
    activeMembers: any[] = [];
    totalRecords = 0;
    page = 1;
    limit = 10;
    ...
    ngOnInit(): void {
     ...
     loadUsers(page: number, limit: number, globalFilter: string = '') {
        this.loading = true;
        this.allUserDetailsService.getallUsers(page, limit, globalFilter,'Active').subscribe({
            next: (data: any) => {
    ...
    exportToExcel() {
        ...
        this.saveAsFile(excelBuffer, 'ActiveMembers', 'xlsx');
    }
    ...
    exportToCSV() {
        ...
        FileSaver.saveAs(blob, `ActiveMembers_${new Date().getTime()}.csv`);
    }

    exportToPDF() {
        const doc = new jsPDF();
        const headers = [['Reg No', 'Name', 'Email', 'Mobile']];
        const rows = this.activeMembers.map(m => [m.registernumber, m.name, m.email, m.mobilenumber]);
        autoTable(doc, { head: headers, body: rows });
        doc.save(`ActiveMembers_${new Date().getTime()}.pdf`);
    }
